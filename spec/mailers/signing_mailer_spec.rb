...
RSpec.describe SigningMailer, type: :mailer do
  describe 'notify' do
    ...
    let!(:service_data) do
      { id: '1', title: 'Тестовое название', description: 'Супер-описание', price: '2000', image: '' }
    end
    ...
    let!(:record_data) do
      { trouble: 'Какая-то проблема', date: '2023-02-02', user_id: '1', service_id: '1', status: 'prepared', master_id: '1',
        service_point_id: '1' }
    end
    let!(:record) { ServiceRecord.create(record_data) }

    let(:mail) { SigningMailer.with(user:, record:).my_record }
