...
RSpec.describe WorkMailer, :type => :mailer do
  describe "notify" do
    ...
    let! (:work_data) {{ user_id: "1", age: '20', male: "women", about: "Какой-то текст здесь", task_1: "Какой-то текст здесь", task_2: 'Какой-то текст здесь', task_3: "Какой-то текст здесь", vacancy_id: "1" }}
    let! (:work) { Work.create(work_data)}
    let! (:vacancy_data) {{id: "1", title: 'Тест'}}
    let! (:vacancy) {Vacancy.create(vacancy_data)}

    let(:mail) { WorkMailer.with(user: user, work: work).my_application_for_work }
